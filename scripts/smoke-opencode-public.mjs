...
const packageMetadata = jsonFile(join(root, 'opencode/package.json'));
const skillsManifest = jsonFile(join(root, 'opencode/skills-manifest.json'));
const productionMcpUrl = `https://aictrl.dev/mcp/workflows/opencode-ecosystem/${packageMetadata.version}/implement-code-change/${skillsManifest.skillsVersion}`;
const connectivityOrigin = process.env.AICTRL_OPENCODE_CONNECTIVITY_ORIGIN || 'https://aictrl.dev';
const connectivityUrl = new URL(new URL(productionMcpUrl).pathname, connectivityOrigin).toString();
