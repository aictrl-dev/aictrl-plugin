...
export async function inspectReleasePublication(
  ...
  if (response.status === 404) {
    return { published: false, name, version };
  }
  if (!response.ok) {
...
async function packPackage(packageSpec) {
  const output = execFileSync('npm', ['pack', packageSpec, '--dry-run', '--json'], {
    cwd: root,
    encoding: 'utf8',
  ...
  if (!result) {
    throw new Error(`npm pack returned no package for ${packageSpec}`);
  }
  return result;
}
...
async function main() {
  ...
  if (!packageSpec) {
    throw new Error('Usage: verify-release-publication.mjs <package-spec>');
  }
