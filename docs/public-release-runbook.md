...
6. Promote the sandbox runtime batch to production and repeat the health, OAuth,
   MCP catalog, schema, annotation, and connected-workflow checks for each
   listing-specific versioned resource generated by package assembly.
...
2. Enter the generated Codex MCP resource URL in the plugin portal:
   `https://aictrl.dev/mcp/workflows/codex-plugin-directory/<plugin-version>/implement-code-change/<skill-version>`.
   Copy it from `plugins/aictrl/.mcp.json`; do not reconstruct or shorten it to
   the generic workflow endpoint.
